...
func (app *Config) ActivateAccount(w http.ResponseWriter, r *http.Request) {
	...
	err = u.Update()
	if err != nil {
		app.Session.Put(r.Context(), "error", "Failed to activate account, unable to update user")
		http.Redirect(w, r, "/", http.StatusSeeOther)
		return
	}

	app.Session.Put(r.Context(), "flash", "Your account has been activated. You can now log in.")
	http.Redirect(w, r, "/login", http.StatusSeeOther)
...
func (app *Config) SubscribeToPlan(w http.ResponseWriter, r *http.Request) {
	...
	go func() {
		...
		err := pdf.OutputFileAndClose(fmt.Sprintf("./tmp/%d-manual.pdf", user.ID))
		if err != nil {
			app.ErrorChan <- err
			return
		}
		...
		msg := Message{
			...
			AttachmentMap: map[string]string{
				"Manual.pdf": fmt.Sprintf("./tmp/%d-manual.pdf", user.ID),
			},
		}
	...
	if err != nil {
		app.Session.Put(r.Context(), "error", "Failed to subscribe user to plan")
		http.Redirect(w, r, "/members/plans", http.StatusSeeOther)
		return
	}
